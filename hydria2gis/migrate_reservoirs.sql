...
/* Insert some new records in gentity generics type
   assuming that only one record alread exist in that
   table (hq curves) */

INSERT INTO hcore_gentitygenericdatatype(id, descr, descr_alt,
                                         file_extension)
       VALUES (2, 'Καμπύλες στάθμης - επιφάνειας - όγκου',
                  'Stage - Area - Storage curves', 'txt');
INSERT INTO hcore_gentitygenericdatatype(id, descr, descr_alt,
                                         file_extension)
       VALUES (3, 'Συντελεστές υπόγειων διαφυγών ταμιευτήρα',
                  'Reservoir leakage coefficients', 'txt');
INSERT INTO hcore_gentitygenericdatatype(id, descr, descr_alt,
                                         file_extension)
       VALUES (4, 'Καμπύλη υπερχείλισης - παροχής ταμιευτήρα',
                  'Reservoir spill - discharge curve', 'txt');

--Import gentities for reservoir
UPDATE hcore_gentity h
    SET (water_basin_id, water_division_id, political_division_id,
        ...
        WHERE h.id in (SELECT a.garea_ptr_id 
                       FROM  gis_objects_gisreservoir a
...
INSERT INTO hcore_gentityfile(gentity_id, descr, descr_alt,
    ...
    FROM gis_objects_gisentity b,
         gis_objects_gisreservoir a,
         oldeydap.gentities_multimedia m
    WHERE m.id=b.original_gentity_id AND a.gisentity_ptr_id = b.id;


/* SPECIAL reservoir FIELDS */


UPDATE gis_objects_gisreservoir r
    SET (inflow_mean, inflow_max, inflow_min, runoff_mean,
         runoff_max, runoff_min, volume_max, dead_volume)=
        (t.inflow_mean, t.inflow_max, t.inflow_min, t.runoff_mean,
         t.runoff_max, t.runoff_min, t.volume_max, t.dead_volume)
        FROM oldeydap.reservoirs t,
...
INSERT INTO hcore_gentitygenericdata(id, gentity_id, descr, descr_alt,
    ...
    SELECT c.id, a.gisentity_ptr_id, COALESCE(c.name, ''), 
           ...
           COALESCE(c.remarks_en, ''), 2, ''
    FROM gis_objects_gisentity b,
         gis_objects_gisreservoir a,
         oldeydap.curves c
    WHERE c.gentity=b.original_gentity_id AND a.gisentity_ptr_id = b.id
          AND c.terminal_subtable='reservoir_hsvb';

--Import leakage coeff. 
INSERT INTO hcore_gentitygenericdata(id, gentity_id, descr, descr_alt,
    remarks, remarks_alt, data_type_id, content)
    SELECT c.id, a.gisentity_ptr_id, COALESCE(c.name, ''), 
           COALESCE(c.name_en, ''), COALESCE(c.remarks, ''), 
           COALESCE(c.remarks_en, ''), 3, ''
    FROM gis_objects_gisentity b,
         gis_objects_gisreservoir a,
         oldeydap.curves c
    WHERE c.gentity=b.original_gentity_id AND a.gisentity_ptr_id = b.id
          AND c.terminal_subtable='reservoir_leakage';

--Import reservoir spill
INSERT INTO hcore_gentitygenericdata(id, gentity_id, descr, descr_alt,
    remarks, remarks_alt, data_type_id, content)
    SELECT c.id, a.gisentity_ptr_id, COALESCE(c.name, ''), 
           COALESCE(c.name_en, ''), COALESCE(c.remarks, ''), 
           COALESCE(c.remarks_en, ''), 4, ''
    FROM gis_objects_gisentity b,
         gis_objects_gisreservoir a,
         oldeydap.curves c
    WHERE c.gentity=b.original_gentity_id AND a.gisentity_ptr_id = b.id
          AND c.terminal_subtable='reservoir_spill';
...
INSERT INTO hcore_timeseries
    (old_id, gentity_id, variable_id, unit_of_measurement_id, name, name_alt,
     ...
     hidden)
     SELECT t.id, a.garea_ptr_id, var, COALESCE(unit, 1001), 
