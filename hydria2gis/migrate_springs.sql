...
/* A: Common Borehole and springs , so this script should
      be run before importing boreholes */

INSERT INTO gis_objects_gisboreholespringwateruse(id, descr, descr_alt)
    SELECT id, COALESCE(descr,''), COALESCE(descr_en,'') 
    FROM oldeydap.water_uses;

INSERT INTO gis_objects_gisboreholespringwateruser(id, descr, descr_alt)
    SELECT id, COALESCE(descr,''), COALESCE(descr_en,'') 
    FROM oldeydap.water_users;

INSERT INTO gis_objects_gisboreholespringlanduse(id, descr, descr_alt)
    SELECT id, COALESCE(descr,''), COALESCE(descr_en,'') 
    FROM oldeydap.land_uses;

/* B: Springs lookup tables */

INSERT INTO gis_objects_gisspringdstype(id, descr, descr_alt)
    SELECT id, COALESCE(descr,''), COALESCE(descr_en,'') 
    FROM oldeydap.dstypes;

INSERT INTO gis_objects_gisspringhgeoinfo(id, descr, descr_alt)
    SELECT id, COALESCE(descr,''), COALESCE(descr_en,'') 
    FROM oldeydap.hgeo_infos;

--Import gentities for spring
...
UPDATE gis_objects_gisboreholespring r
    SET (water_use_id, water_user_id, land_use_id, continuous_flow)=
        ...
        FROM oldeydap.boreholes_springs t,
             ...
             gis_objects_gisspring s
        WHERE s.gisentity_ptr_id=a.id
        AND r.gpoint_ptr_id = s.gisboreholespring_ptr_id
        AND a.gtype_id=4
        AND t.id=a.original_gentity_id;

UPDATE gis_objects_gisspring r
    SET (dstype_id, hgeo_info_id, is_continuous)=
        (t.dstype, t.hgeo_info,
        COALESCE(t.is_continuous, False))
        FROM oldeydap.springs t,
             gis_objects_gisentity a
...
SELECT update_sequence('gis_objects_gisboreholespringwateruse_id_seq', 
                       'gis_objects_gisboreholespringwateruse');
SELECT update_sequence('gis_objects_gisboreholespringwateruser_id_seq', 
                       'gis_objects_gisboreholespringwateruser');
SELECT update_sequence('gis_objects_gisboreholespringlanduse_id_seq', 
                       'gis_objects_gisboreholespringlanduse');
SELECT update_sequence('gis_objects_gisspringdstype_id_seq', 
                       'gis_objects_gisspringdstype');
SELECT update_sequence('gis_objects_gisspringhgeoinfo_id_seq', 
                       'gis_objects_gisspringhgeoinfo');
