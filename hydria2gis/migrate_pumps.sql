...
INSERT INTO gis_objects_gispumptype(id, descr, descr_alt)
    SELECT id, COALESCE(descr,''), COALESCE(descr_en,'') 
    FROM oldeydap.pump_types;


--Import gentities for pump
...
INSERT INTO hcore_gentityfile(gentity_id, descr, descr_alt,
    ...
    FROM gis_objects_gisentity b,
         gis_objects_gispump a,
         oldeydap.gentities_multimedia m
    WHERE m.id=b.gis_id AND a.gisentity_ptr_id = b.id;


/* SPECIAL PUMPS FIELDS */


UPDATE gis_objects_gispump r
    SET (pump_type_id, is_irrig, is_pump, pump_active, is_generator,
         generator_active, pump_num, pump_discharge, generator_num, 
         total_power, generator_discharge, pump_energy, 
         generator_energy)=
         (t.pump_type, t.is_irrig, t.is_pump, t.pump_active, 
         t.is_generator, t.generator_active, t.pump_num, 
         t.pump_discharge, t.generator_num, t.total_power, 
         t.generator_discharge, t.pump_energy, t.generator_energy)
    FROM oldeydap.pumps_generators t,
...
SELECT update_sequence('gis_objects_gispumptype_id_seq',
                       'gis_objects_gispumptype');
